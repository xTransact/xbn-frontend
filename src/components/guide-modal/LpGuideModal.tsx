...
const StepImageComponent: FunctionComponent<{ order: number }> = ({
  ...
  console.log(
    '🚀 ~ file: LpGuideModal.tsx:48 ~ StepImageComponent ~ key:',
    order,
  )
  switch (order) {
    case 1:
...
const LpGuideModal: FunctionComponent<{
  onClose: () => void
  isOpen: boolean
}> = ({ onClose, ...rest }) => {
  const [step, setStep] = useState<number>(1)
  const { title, index, description } = useMemo(() => {
    return LP_GUIDES[step - 1]
  }, [step])
  return (
        ...
        >
          ...
          >
            <Button
              w='158px'
              h='40px'
              variant={step === LP_GUIDES.length ? 'primary' : 'outline'}
              px='30px'
              onClick={() => {
                console.log('aaaaaaaa')
                if (step === LP_GUIDES.length) {
                  setStep(1)
                  onClose()
                }
                setStep((prev) => prev + 1)
            ...
            >
              {step < LP_GUIDES.length ? 'Next' : 'Continue'}
            </Button>
          </Flex>
          <Flex
            pb={{
